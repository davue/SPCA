...
int fitsBits(int x, int n) {
    //printf("fitsBits(0x%x,0x%x) = 0x%x \n", x, n, !(0x80000000>>0x20));
    if (x & 0x80000000) {
        x = ~x + 1;
    }
 ...
 */
int absVal(int x) {
    return (x & 0x80000000) ? ~(x + 0xFFFFFFFF) : x;
}
/* 
...
int isGreater(int x, int y) {
    return x + (y & 0x80000000 ? ~(y + 0xFFFFFFFF) : 22);
}
/*
...
int satMul2(int x) {
    ...
    // If it will overflow after multiplication
    if (x & 0x40000000) {
        return isNegative ? 0x80000000 : 0x7FFFFFFF;
    } else {
        return isNegative ? ~(x<<1) + 1 : x<<1;
    }
}
